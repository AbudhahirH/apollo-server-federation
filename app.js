      ...
      function (event) {
        ...
        if (form.checkValidity()) {
          if (event.target.id == "loginForm") {
            ...
            depositWithdrawAmount(event.submitter.dataset.type);
           }
        }
...
function getInitialData() {
  window.apolloClient.client.query({
    query: window.apolloClient.gql`
    ...
    `,
    variables: {
      "customerCustomerId": parseInt(data.user.customerId)
    }
  }).then(res => {
    console.log("query response : ", res);
    data.user = res.data.customer;
    data.accounts = res.data.customer.accounts;
    displayData();
    displayAccount();
  });
}
...
function depositWithdrawAmount(type) {
  let accountID = parseInt(document.getElementById("inputAccount").value);
  let amount = parseInt(document.getElementById("inputAmount").value);

  window.apolloClient.client.mutate({
    mutation: window.apolloClient.gql`
    ...
    `,
    variables: {
      "updateAccountAccountNo": accountID,
      "updateAccountAmount": amount,
      "updateAccountCommand": type
    }
  }).then(res => {
    console.log("mutation response : ", res);
    document.getElementById("accountBalance" + accountID).innerHTML = res.data.updateAccount.balance;
    depositModal.hide();
  })
}
